...
class CardioChatService:
    ...
    def _build_prompt(
        ...
        if not supplements_data:
            ...
            instruction_tone = "Используй ТОЛЬКО данные из списка ниже. Не выдумывай несуществующие препараты."
            supp_text = "--- РАЗРЕШЕННЫЙ СПИСОК НУТРИЕНТОВ (Knowledge Base) ---\n"
            for name, data in supplements_data.items():
                supp_text += f"- {data.get('name')}: {data.get('mechanism','')}. (Меры предосторожности: {data.get('warnings','-')})\n"
        ...
        prompt = (
            ...
            f"ИНСТРУКЦИИ:\n"
            f"1. Дай ОДНУ конкретную, выполнимую рекомендацию по образу жизни (персонализированную под возраст и активность).\n"
            f"2. {instruction_tone} Если список ниже не пуст, выбери 2-3 наиболее подходящих пунктов и кратко опиши их пользу.\n\n"
            f"{supp_text}\n\n"
            f"ВАЖНО: Закончи ответ фразой: 'Данная информация носит ознакомительный характер и не заменяет консультацию врача.'"
        )
        return prompt
